...
**UI / styling.** `src/App.css` is a token-based design system (dark "console" look) — all color reads off accent CSS custom properties (`--acc`, `--acc2`, `--glow`, `--g0..g2`, `--txt`/`--dim`/`--mute`) swapped by a `data-theme` attribute on `<html>`. Accent themes are defined in `src/lib/theme.ts` (`applyTheme`; default `ember`, plus `cobalt`/`emerald`/`plasma`), persisted via the `theme` setting, and applied in `App.tsx` on load (use `||` fallback, not `??` — empty-string trap). Style new UI with the existing tokens and shared classes (`.card`/`.rec-hero`, `.gauge`, `.taste`, `.primary`, `.tab`, `.origin`, `.hint`) — don't hardcode colors or reintroduce per-component palettes.
